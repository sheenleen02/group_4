...
const Product = () => {
  ...
  const handleAddProduct = () => {
    if (isEditing) {
      setProducts(products.map(product =>
        product.id === currentProductId ? { ...product, ...newProduct, id: currentProductId } : product
      ));
      setIsEditing(false);
    } else {
  ...
  return (
    <section className="flex flex-col items-center p-4 bg-white">
      {/* Category Navigation */}
      <nav className="flex items-center gap-4 w-full mb-4 flex-wrap">
        <button className="text-black bg-transparent border-none font-medium">Edit by Categories</button>
        <div className="flex items-center justify-center flex-wrap">
          {categories.map(category => (
            <button
              key={category.name}
              className={`flex items-center justify-between gap-2 p-2 transition-all duration-200 rounded ${
      ...
      <button
        ...
        onClick={() => { setShowModal(true); setIsEditing(false); setNewProduct({ name: '', price: '', description: '', category: '', stock: 1, hide: false, image: '' }); }}
      >
        + Add Product
      </button>

      {/* Product Display */}
      <div className="grid w-full grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 sm:grid-cols-3">
        {filteredProducts.map(product => (
          <div key={product.id} className="bg-gray-300 p-4 shadow-lg rounded flex flex-col items-center">
            ...
            <div className='flex justify-between gap-4'>
              <button onClick={() => handleEditProduct(product)} className="bg-blue-900 text-white px-4 py-2 rounded mt-2">Edit</button>
              <button onClick={() => handleRemoveProduct(product.id)} className="bg-red-500 text-white px-4 py-2 rounded mt-1">Remove</button>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
          <div className="bg-gray-300 p-8 rounded-lg shadow-lg w-[90%] md:w-[600px]">
            ...
            <div className="space-y-4">
              ...
              <button onClick={handleAddProduct} className="bg-black text-white px-4 py-2 rounded w-full mt-4">{isEditing ? 'Update Product' : 'Add Product'}</button>
            </div>
          </div>
        </div>
      )}
